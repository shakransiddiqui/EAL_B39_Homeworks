package rabiul_homework;

/* Array Task 3 :
● Create an String array without limiting the index length(Variable Name should
not be single Digit,it can be a name or anything but not a ,b, c ,d) and Put 5
String Value there. */


public class Java_2_3_AT_3_P_11 {

	public static void main(String[] args) {
		
		String sap[]= {"Mohammad", "Rabiul", "Awal", "Bhuiyan", "Sujon"};
		
		System.out.println("Question-1: Print all the value with for each loop");
		
		for(String x:sap) {
	 		System.out.println(x);
	 	}
		
		System.out.println("Question-2:Print the value of index number 2and index 3 individually. (Be careful with the index\r\n"
				+ "name.Index Name and Index number are not same.)");
		
		System.out.println(sap[1]);
	 	System.out.println(sap[2]);
	 	
	 	
	 	System.out.println("Question-3:Print all the value from 1rst index to last index with For loop");
	 	
	 	for(int x=0; x<sap.length;x++) {
	 		
	 		System.out.println(sap[x]);
	 	}
	 	
	 	System.out.println("Question-4:Print all the value from last Index (a.length-1) to first Index (Printing should be in reverse\r\n"+ "order");
	 	
	 	for(int x=sap.length-1;x>=0;x--) {
	 		
	 		System.out.println(sap[x]);
	 	}
	 	
	 	System.out.println("Question-5:Print all the value 5th index to 2nd index.");
	 	
	 	for(int x=4;x>1;x--) {
			
			System.out.println(sap[x]);
		}
	 	
	 	
	 	
	}

}
